...
| `quality/deep-review` | review | Escalated maintainability/abstraction audit (severity+confidence) |
| `quality/deslop` | review | Diff-scoped redundant code cleanup (pairs with `refactor-cleaner` specialist) |
| `quality/unslop` | review | Prose/docs anti-slop before final publish (pairs with docs touched by `implementer`) |

Specialists that may back you (opt-in, not replacement): `code-reviewer`, `refactor-cleaner`, `typescript-reviewer` (includes `database-reviewer`/`performance-optimizer` concerns via deep-review). No review skill claims full compliance from automated checks alone.
...
| Escalated craft/abstraction audit | `quality/deep-review` (pairs with `code-reviewer` agent) |
| Code slop in diff hunks | `quality/deslop` |

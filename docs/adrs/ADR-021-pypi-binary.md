...
- Missing bundled binary in an editable/dev tree is an error pointing at `./make.vsh build-cli` — not a silent Python fallback.
...
CI copies GitHub Release V binaries into the wheel (`scripts/pack_pypi.vsh` + `scripts/prepare-native-bin.vsh`). The repo root is not a uv workspace; `uv.lock` belongs next to the adapter if present. Pre-commit Ruff uses `language: python` (`ruff-pre-commit`) with root `ruff.toml` — no product uv workspace.

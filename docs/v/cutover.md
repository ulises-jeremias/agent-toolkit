...
EPIC 5 remaining commands (`project`, `loop`, `swarm`, `devcompanion`, `insights`, `release`) may still be `not_implemented` in V. `workspace` (#520) and `memory` (#521) are implemented in V. Run the Python CLI explicitly for unfinished advanced commands:

...
| PyInstaller→native cutover gaps | M | H | V assets on GitHub Release since `v1.11.0` ([#530](https://github.com/ulises-jeremias/agent-toolkit/issues/530)); Python wheel still publishes as launcher | Release smoke | Keep launcher wheel only (Python CLI removed v1.13.0) | **Mitigated** `v1.13.0` |
| Dual-engine drift | H | H | Golden `tests/parity/`; short dual period; ADR-012 | Parity CI | Product path is V-only; Python CLI removed v1.13.0 | **Mitigated** [#540](https://github.com/ulises-jeremias/agent-toolkit/issues/540) / `v1.13.0` |

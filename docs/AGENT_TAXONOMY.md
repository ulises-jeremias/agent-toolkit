...
> Authority: `capabilities/skills/registry.yaml` (`schemas/skill-capability-registry.schema.json` — 11 `holistic_owner` values, 103+ skills, no orphans) + `docs/SKILL_ROUTING.md` + `skills/core/assistant/references/ORCHESTRATION.md`.
> Validate with `./scripts/validate-skill-capability.vsh --check`, `./scripts/validate-agents.vsh`, `./scripts/generate-catalogs.vsh --check`.
...
./scripts/validate-skill-capability.vsh --check   # 103+ skills, no orphans, design routing still passes

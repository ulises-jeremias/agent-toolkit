...
The Cursor marketplace manifest is at `.cursor-plugin/marketplace.json` in the repository. It is
generated by `agent-toolkit build` from `distributions/products.yaml` and lists `core`, `agents`,
and `forge` with schema fields only (`name`, `source`, `description`, `version`, `category`,
`tags`).

Per-plugin Cursor manifests live at `plugins/<name>/.cursor-plugin/plugin.json` and include
`displayName`, `category`, `tags`, and explicit `skills` / `agents` glob paths when those
components ship in the bundle.

### plugin.json format (Cursor)

```json
{
  "name": "agent-toolkit-core",
  "displayName": "Agent Toolkit Core",
  "version": "1.0.0",

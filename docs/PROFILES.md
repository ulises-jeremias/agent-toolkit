# Profiles — deprecated install overlay (ADR-004)

> **Status:** `profiles/` is a **deprecated install overlay** — not the canonical skill source of truth. `plugins/` (generated by `agent-toolkit build` from `distributions/products.yaml` + `skills/`/`agents/`) is canonical (`build --check` enforces digests). Install resolution in `modules/agent_toolkit_core/install.v` prefers compiled `plugins/` when present, else falls back to `profiles/`. Add new capabilities to `skills/` + `products.yaml`, not `profiles/` alone. See `docs/ARCHITECTURE.md` (Capability Plane) and `docs/CONCEPTS.md` (Two Planes).

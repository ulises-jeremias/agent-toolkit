...
#
# Examples:
#   # Build with default name (jira-skills-dev-enhanced:latest)
#   ./build-enhanced.sh
#
...
IMAGE_NAME="jira-skills-dev-enhanced"
IMAGE_TAG="latest"
PUSH_IMAGE=false
CA_CERT=""
NO_CACHE=false
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo ""
            echo "Options:"
            echo "  --image NAME    Image name (default: jira-skills-dev-enhanced)"
            ...
            echo "  ./run_devcontainer.sh --use-enhanced"
            echo "  ./run_devcontainer.sh --image $IMAGE_NAME --tag $IMAGE_TAG"
            exit 0
            ;;
        *)
...
if [[ -n "$CA_CERT" ]]; then
    if [[ ! -f "$SCRIPT_DIR/$CA_CERT" ]]; then
        echo_error "CA certificate not found: $SCRIPT_DIR/$CA_CERT"
        exit 1
    fi
...
docker build \
    ...
    -f "$SCRIPT_DIR/Dockerfile.dev-enhanced" \
    "$SCRIPT_DIR"
